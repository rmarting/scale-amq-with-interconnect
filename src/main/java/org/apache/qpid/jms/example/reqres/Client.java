 ...
 *
 */
package org.apache.qpid.jms.example.reqres;
...
public class Client {
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			ConnectionFactory factory = (ConnectionFactory) context.lookup("requestResponseFactory");
			...
			System.out.println("[CLIENT] Connected to " + ((JmsConnection) connection).getConfiguredURI());

			// Create a temporary queue and consumer to receive responses, and a producer to
			// send requests.
			Destination queue = (Destination) context.lookup("requestQueue");
			...
			String[] requests = new String[] { "Twas brillig, and the slithy toves", "Did gire and gymble in the wabe.",
			...
			for (String request : requests) {
				...
				messageProducer.send(requestMessage, DeliveryMode.NON_PERSISTENT, Message.DEFAULT_PRIORITY,
				...
				if (responseMessage != null) {
					...
					System.out.println(
							"[CLIENT] Response for '" + request + "' was not received within the timeout, exiting.");
					break;
				}
			}
